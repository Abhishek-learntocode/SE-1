...
if (Test-Path $PATH_TO_CHECKSTYLE) {
    ...
    $CONFIG = if (Test-Path $PATH_TO_SUN_CHECKS) { $PATH_TO_SUN_CHECKS } else { "/sun_checks.xml" }
    java -jar "$PATH_TO_CHECKSTYLE" -c "$CONFIG" -f xml -o "$SNAPSHOT_ABS\checkstyle_report.xml" "$SourcePath" 2>&1 | Out-Null
    if (Test-Path "$SNAPSHOT_DIR\checkstyle_report.xml") {
        Write-Host "      [OK] checkstyle_report.xml generated" -ForegroundColor Green
    } else {
        Write-Host "      [WARN] Checkstyle report not generated" -ForegroundColor DarkYellow
    }
} else {
    Write-Host "[SKIP] Checkstyle jar not found" -ForegroundColor DarkGray
}

# 1c. PMD
if (Test-Path $PATH_TO_PMD_BAT) {
    Write-Host "[3/4] Running PMD..." -ForegroundColor Yellow
    & "$PATH_TO_PMD_BAT" check -d "$SourcePath" -R rulesets/java/quickstart.xml -f xml -r "$SNAPSHOT_ABS\pmd_report.xml" 2>&1 | Out-Null
    if (Test-Path "$SNAPSHOT_DIR\pmd_report.xml") {
