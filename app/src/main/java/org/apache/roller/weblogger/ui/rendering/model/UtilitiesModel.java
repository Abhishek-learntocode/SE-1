...
import org.apache.roller.weblogger.ui.rendering.util.WeblogRequestContext;
...
public class UtilitiesModel implements Model {
    ...
    private WeblogRequestContext parsedRequest = null;
    ...
    public void init(Map<String, Object> initData) throws WebloggerException {      
        ...
        Object requestObj = initData.get("parsedRequest");
        if(requestObj instanceof WeblogRequestContext) {
            parsedRequest = (WeblogRequestContext) requestObj;
        }
        ...
        weblog = parsedRequest.getWeblog();
    }
        ...
        try {
            if (parsedRequest.getAuthenticUser() != null) {
                return WebloggerFactory.getWeblogger().getUserManager().checkPermission(
                        new WeblogPermission(weblog.getPojo(), parsedRequest.getUser(), Collections.singletonList(WeblogPermission.POST)), parsedRequest.getUser());
            }
        } catch (Exception e) {
    ...
    public boolean isUserAuthorizedToAdmin(WeblogWrapper weblog) {
        try {
            if (parsedRequest.getAuthenticUser() != null) {
                return WebloggerFactory.getWeblogger().getUserManager().checkPermission(
                        new WeblogPermission(weblog.getPojo(), parsedRequest.getUser(), Collections.singletonList(WeblogPermission.ADMIN)), parsedRequest.getUser());
            }
        } catch (Exception e) {
            log.warn("ERROR: checking user authorization", e);
        }
