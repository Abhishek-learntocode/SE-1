...
public class CategoryEdit extends UIAction {
    ...
    public void myValidate() {
        if (bean.getName() == null || !bean.getName().equals(StringEscapeUtils.escapeHtml4(bean.getName()))) {
            ...
            WeblogCategory wc = null;
            try {
                wc = WebloggerFactory.getWeblogger().getWeblogEntryManager()
                    .getWeblogCategoryByName(getActionWeblog(), bean.getName());
            } catch (WebloggerException ex) {
                addError("categoryForm.error.checking");
            }
