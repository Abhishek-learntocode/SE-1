...
public class JPAWeblogEntryManagerImpl implements WeblogEntryManager {
    ...
    private final WeblogTagRepository weblogTagRepository;
    ...
    protected JPAWeblogEntryManagerImpl(Weblogger roller, JPAPersistenceStrategy strategy, CommentManager commentManager, WeblogTagRepository weblogTagRepository) {
        ...
        this.weblogTagRepository = weblogTagRepository;
    }
    ...
    public void saveWeblogEntry(WeblogEntry entry) throws WebloggerException {
        ...
        if (entry.isPublished()) {
            ...
            if (entry.getRefreshAggregates()) {
                ...
                for (WeblogEntryTag tag : entry.getTags()) {
                    weblogTagRepository.updateTagCount(tag.getName(), entry.getWebsite(), 1);
                }
            } else {
        ...
        for (WeblogEntryTag tag : entry.getRemovedTags()) {
            weblogTagRepository.removeWeblogEntryTag(tag);
        }
    ...
    public List<WeblogEntryComment> getComments(CommentSearchCriteria csc) throws WebloggerException {
        return this.commentManager.getComments(csc);
    }
    ...
    public int removeMatchingComments(
        ...
        return this.commentManager.removeMatchingComments(weblog, entry, searchString, startDate, endDate, status);
    }
    ...
    public long getCommentCount() throws WebloggerException {
        return this.commentManager.getCommentCount();
    }
    ...
    public long getCommentCount(Weblog website) throws WebloggerException {
        return this.commentManager.getCommentCount(website);
    }
