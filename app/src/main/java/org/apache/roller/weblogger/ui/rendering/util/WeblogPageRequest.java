 ...
 *
 * Uses composition with WeblogRequest rather than inheritance,
 * delegating common request handling to the composed instance.
 */
public class WeblogPageRequest implements WeblogRequestContext {
    ...
    // composed base request — replaces inheritance from WeblogRequest
    ...
    public WeblogPageRequest() {
        this.baseRequest = new WeblogRequest();
    }
    ...
    public WeblogPageRequest(HttpServletRequest request)
        ...
        // delegate common parsing (weblog handle, locale, path info) to composed WeblogRequest
        this.baseRequest = new WeblogRequest(request);

        String servlet = request.getServletPath();

        // we only want the path info left over from after the base request's parsing
        String pathInfo = this.baseRequest.getPathInfo();
        ...
        // parse path info to extract context, anchor, date, category, page, tags
        parsePathInfo(pathInfo, request);

        // parse request parameters for entry, anchor, date, cat
        parseRequestParameters(pathInfo, request);

        // page request param is supported in all views
        parsePageNumber(request);
    ...
    /**
     * Parse path info to extract context, anchor, date, category, page name, or tags.
     *
    ...
    /**
     * Parse request parameters for entry, anchor, date, and category.
     *
     ...
     */
    private void parseRequestParameters(String pathInfo, HttpServletRequest request)
            throws InvalidRequestException {
     ...
     */
    private void parsePageNumber(HttpServletRequest request) {
    ...
    /**
     * Build custom parameters map, excluding built-in params.
     */
    private void buildCustomParams(HttpServletRequest request) {
    ...
    // ----------------------------------------------------------------
    // Delegation methods — WeblogRequest (common request handling)
    // ----------------------------------------------------------------

    @Override
    public String getWeblogHandle() {
        return baseRequest.getWeblogHandle();
    }

    public void setWeblogHandle(String weblogHandle) {
        baseRequest.setWeblogHandle(weblogHandle);
    }
    ...
    public Weblog getWeblog() {
        return baseRequest.getWeblog();
    }

    public void setWeblog(Weblog weblog) {
        baseRequest.setWeblog(weblog);
    }

    @Override
    public Locale getLocaleInstance() {
        return baseRequest.getLocaleInstance();
    }

    public void setLocaleInstance(Locale localeInstance) {
        baseRequest.setLocaleInstance(localeInstance);
    }

    // ----------------------------------------------------------------
    // Delegation methods — ParsedRequest (authentication, device type)
    // ----------------------------------------------------------------

    @Override
    public String getAuthenticUser() {
        return baseRequest.getAuthenticUser();
    }

    public void setAuthenticUser(String authenticUser) {
        baseRequest.setAuthenticUser(authenticUser);
    }

    @Override
    public User getUser() {
        return baseRequest.getUser();
    }
    ...
    public boolean isLoggedIn() {
        return baseRequest.isLoggedIn();
    }

    public MobileDeviceRepository.DeviceType getDeviceType() {
        return baseRequest.getDeviceType();
    }

    public void setDeviceType(MobileDeviceRepository.DeviceType type) {
        baseRequest.setDeviceType(type);
    }

    // ----------------------------------------------------------------
    // WeblogPageRequest-specific getters/setters
    // ----------------------------------------------------------------
    ...
    public ThemeTemplate getWeblogPage() {
        ...
        if (weblogPage == null && weblogPageName != null) {
            try {
                weblogPage = WebloggerFactory.getWeblogger().getThemeManager().getTheme(getWeblog()).getTemplateByLink(
                        weblogPageName);
            } catch (WebloggerException ex) {
