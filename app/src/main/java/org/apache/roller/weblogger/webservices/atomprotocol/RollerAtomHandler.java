 ...
 */
public class RollerAtomHandler implements AtomHandler, AtomRequestContext {
    ...
    //------------------------------------------------------------------ AtomRequestContext implementation
     ...
     */
    @Override
    public boolean canEdit(User u, WeblogEntry entry) {
        try {
     ...
     */
    @Override
    public boolean canEdit(User u, Weblog website) {
        try {
            return WebloggerFactory.getWeblogger().getUserManager().checkPermission(
                    new WeblogPermission(website, u, Collections.singletonList(WeblogPermission.POST)), u);
        } catch (Exception e) {
     ...
     */
    @Override
    public boolean canView(User u, WeblogEntry entry) {
     ...
     */
    @Override
    public boolean canView(User u, Weblog website) {
    ...
    @Override
    public void oneSecondThrottle() {
        // Throttle one entry per second per weblog because time-
        // stamp in MySQL and other DBs has only 1 sec resolution
        if (THROTTLE) {
            try {
                // Synchronize on class object (not instance) to throttle
                // across all concurrent requests, preserving original semantics
